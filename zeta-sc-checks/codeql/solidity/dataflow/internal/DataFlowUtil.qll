 ...
 *
 * Provides Solidity-specific definitions for use in the data flow library.
 */

// private import cpp
private import codeql.solidity.ast.internal.TreeSitter
private import codeql.Locations
// private import codeql.solidity.dataflow.internal.FlowVar
private import codeql.solidity.models.interfaces.DataFlow
// private import semmle.code.cpp.controlflow.Guards
// private import AddressFlow
import codeql.solidity.Function
import codeql.solidity.exprs.Call
import codeql.solidity.controlflow.BasicBlocks
import codeql.solidity.Assignment
import codeql.solidity.Field
import codeql.solidity.dataflow.internal.FlowVar
// import codeql.dataflow.DataFlow
// import codeql.solidity.exprs.Access


cached
private newtype TNode =
  TExprNode(Expression e)
  or
  TArgumentNode(CallArgument e) or
...
class Node extends TNode {
   ...
   * `asExpr`.
   */
  Expression asExpr() { result = this.(ExprNode).getExpr() }

  /** Gets the parameter corresponding to this node, if any. */
  Parameter asParameter() { result = this.(ExplicitParameterNode).getParameter() }

  CallArgument asArgument() {result = this.(ArgNode).getArgument()}

  LocalVariable asLocalVariable() {result = this.(UninitializedNode).getLocalVariable()}
  ...
  //  */
  // Solidity::Expression asDefiningArgument() { result = this.(DefinitionByReferenceNode).getArgument() }
...
class ExprNode extends Node, TExprNode {
  ...
  Expression getExpr() { result = expr }
}


/**
 * NOTA ARGI: agrego esta clase!
 * 
 * The value of a call argument, viewed as a node in the 
 * flow graph.
 */
class ArgNode extends Node, TArgumentNode {
  // Parameter param;
  CallArgument arg;

  ArgNode() { this = TArgumentNode(arg) }

  override Function getFunction() { result = arg.getParent() }

  // override Type getType() { result = param.getType() }

  // override Location getLocation() { result = param.getLocation() }
  override Location getLocation() { result = arg.getParent().getLocation() }

  /** Gets the parameter corresponding to this node. */
  // Parameter getParameter() { result = param }
  CallArgument getArgument() { result = arg }
}
 ...
 * flow graph.
 */
class ExplicitParameterNode extends Node, TExplicitParameterNode {
  Parameter param;
  ...
  // override Location getLocation() { result = param.getLocation() }
  override Location getLocation() { result = param.(AstNode).getLocation() }
 ...
 */
ExprNode exprNode(Expression e) { result.getExpr() = e }
 ...
 */
pragma[inline]
predicate localExprFlow(Expression e1, Expression e2) { localFlow(exprNode(e1), exprNode(e2)) }
 ...
 */
private predicate exprToVarStep(Expression assignedExpr, FlowVar var) {
  exists(ControlFlowNode operation |
    var.definedByExpr(assignedExpr, operation) 
    // and
 ...
 */
// private predicate exprToExprStep_nocfg(Expression fromExpr, Expression toExpr) {
predicate exprToExprStep_nocfg(Expression fromExpr, Expression toExpr) {
...
// VariableAccess getAnAccessToAssignedVariable(Expr assign) {
  // VariableAccess getAnAccessToAssignedVariable(Solidity::Expression assign) {
  // (
