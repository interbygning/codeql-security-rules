...
// import cpp
private import codeql.solidity.controlflow.SSA
private import SubBasicBlocks
private import codeql.solidity.dataflow.EscapesTree
...
class FlowVar extends TFlowVar {
   ...
   */
  cached
  abstract predicate definedByExpr(Expression e, ControlFlowNode node);
   ...
   */
  cached
  abstract predicate definedPartiallyAt(Expression e);
...
private module PartialDefinitions {
  abstract class PartialDefinition extends Expression {
     ...
     * - `inner` = `a`, `outer` = `*`
     */
    abstract predicate definesExpressions(Expression inner, Expression outer);
    ...
    Location getActualLocation() {
      ...
      // and not result instanceof UnknownLocation
    }
  }
  ...
  private Expression getInnerDefinedExpr(Expression e, ControlFlowNode node) {
    result = e
    // // not e instanceof Conversion and
    // exists(Expression convertedInner |
    //   // valueToUpdate(convertedInner, e.getFullyConverted(), node) and
    //   valueToUpdate(convertedInner, e, node) and
  ...
  class VariablePartialDefinition extends PartialDefinition {
     ...
     * - `inner` = `a`, `outer` = `*`
     */
    override predicate definesExpressions(Expression inner, Expression outer) {
      inner = innerDefinedExpr and
      outer = this
    }
...
module FlowVar_internal {
  ...
  predicate fullySupportedSsaVariable(Variable v) {
    ...
    // loop.
    not exists(AlwaysTrueUponEntryLoop loop | loop.alwaysAssignsBeforeLeavingCondition(_, _, v)) 
    // and
  ...
  predicate blockVarDefinedByVariable(SubBasicBlock sbb, StackVariable v) {
    ...
    exists(DeclStmt declStmt |
      ...
      declStmt.getAVariable() = v.(LocalVariable) and
  ...
  class SsaVar extends TSsaVar, FlowVar {
    ...
    override predicate definedByExpr(Expression e, ControlFlowNode node) {
    ...
    override predicate definedPartiallyAt(Expression e) { none() }
  ...
  class BlockVar extends TBlockVar, FlowVar {
    ...
    override VariableAccess getAnAccess() {
      ...
      result != sbb
      or
      // parameterUsedInConstructorFieldInit(v, result) 
      // and
      sbb = v.(Parameter).getFunction().getEntryPoint()
    }
    ...
    override predicate definedPartiallyAt(Expression e) {
  ...
  // private ControlFlowNode getCfnParent(ControlFlowNode node) { result = node.getEnclosingStmt() }
  private ControlFlowNode getCfnParent(ControlFlowNode node) { result = node.getParent()}
  ...
  private predicate variableNotLiveBefore(SubBasicBlock sbb, Variable v) {
    ...
    exists(DeclStmt ds | ds.getAVariable().(LocalVariable) = v and sbb.contains(ds))
  }
  ...
  predicate notAccessedAtStartOfBB(UninitializedLocalVariable v, BasicBlock bb) {
    ...
    bb.getANode().(DeclStmt).getAVariable() = v and
   ...
   * predicate.
   */
  predicate assignmentLikeOperation(ControlFlowNode node, Variable v, Expression assignedExpr) {
   ...
   */
  predicate initializer(LocalVariable v, Expression assignedExpr) {
