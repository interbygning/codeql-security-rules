...
class TAccess = TAssignment or TCallArgument;

/**
 * A C/C++ access expression. This refers to a variable, or enum constant.
 */
// class Access extends Expr, NameQualifiableElement, @access {
class Access extends Expression, TAccess {
  AstNode e;
  AstNode getUnderlyingExp(){result = e}  //TODO: agrego esto. HACKY. Reestructurar despues

  Access() { toTreeSitter(e) instanceof Solidity::AssignmentExpression or 
    toTreeSitter(e) instanceof Solidity::AugmentedAssignmentExpression or
    toTreeSitter(e) instanceof Solidity::EnumValue or
    toTreeSitter(e) instanceof Solidity::EnumDeclaration or
    toTreeSitter(e) instanceof Solidity::VariableDeclaration
  }

  /** Gets the accessed function, variable, or enum constant. */
  Variable getTarget() { none() } // overridden in subclasses
...
//  * ```
//  */
// class EnumConstantAccess extends Access {
...
class VariableAccess extends Access {
  ...
  override Variable getTarget() { exists(Variable v |
    v.getName() = toTreeSitter(this.getUnderlyingExp()).(Solidity::VariableDeclaration).getName().getValue() or
    v.getName() = toTreeSitter(this.getUnderlyingExp()).(Solidity::AssignmentExpression).getLeft().getChild().(Solidity::Identifier).getValue()
    | result = v // TODO: Implement the rest of possible access
    ) }
  ...
  predicate isRValue() {
    not exists(AssignExpr ae | ae.getLValue() = this) 
    // and
   ...
   */
  Expression getQualifier() { this.getChild(-1) = result }
...
class FieldAccess extends VariableAccess {
  ...
  FieldAccess() { exists(Field f | this.(AssignExpr).getLValue().getChild(_) = f )}
...
class DotFieldAccess extends FieldAccess {
  ...
  // DotFieldAccess() { this.getQualifier().getUnspecifiedType() instanceof TContractDeclaration }
  DotFieldAccess() { exists(this.getQualifier())}
}
 ...
 * ```
 */
class TypeName extends Solidity::TypeName {
