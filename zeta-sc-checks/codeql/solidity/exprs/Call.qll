...
class Call extends Expression, TCallExpression {
  private Solidity::CallExpression node;

  Call(){toTreeSitter(this) = node}
   ...
   */
  // Solidity::CallArgument getAnArgument() { exists(int i | result = toTreeSitter(this.getChild(i))) }
  CallArgument getAnArgument() { exists(int i | result = this.getChild(i)) }
   ...
   */
  CallArgument getArgument(int n) { result = this.getChild(n)}
   ...
   * - For a Solidity method call, it's the statically resolved method.
   */
...
class FunctionCall extends Call {
  ...
  override Function getTarget() { 
    exists(Function f | f.getName() = toTreeSitter(this).(Solidity::CallExpression).getFunction().getChild().(Solidity::Identifier).getValue()
    and result = f)
    //or 
    //TODO:...manejar el caso de MemberExpression y el caso de NewExpression
  }
  ...
  override string toString() {
    ...
    then result = "call to " + toTreeSitter(this.getTarget()).(Solidity::FunctionDefinition).getName()
...
class CallArgument extends Expression, TCallArgument{
  private Solidity::CallArgument node;

  CallArgument(){toTreeSitter(this) = node}
}