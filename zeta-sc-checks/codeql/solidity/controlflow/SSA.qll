...
import codeql.solidity.controlflow.SSAUtils
...
class SsaDefinition extends ControlFlowNodeBase {
  ...
  override Location getLocation() { result = this.(ControlFlowNode).getLocation() }
   ...
   * respectively.
   */
  Expression getDefiningValue(StackVariable v) {
   ...
   */
  Expression getAnUltimateDefiningValue(StackVariable v) {
