...
private class Node extends ControlFlowNodeBase {
  ...
  final Node getParentNode() {
    result = this.(Expression).getParent()
    or
 ...
 */
private predicate excludeNodeAndNodesBelow(AstNode e) {
  not exists(e.getParent()) 
  // and
...
private class PostOrderNode extends Node {
  PostOrderNode() {
    // this instanceof Solidity::Expression 
    toTreeSitter(this) instanceof Solidity::Expression 
    // and
    // not this instanceof Solidity::Expr and
...
private class PreOrderNode extends Node {
  PreOrderNode() {
    ...
    // or
    toTreeSitter(this) instanceof Solidity::VariableDeclaration  //TODO: generic declarationStmt
    or
...
private Node getControlOrderChildSparse(Node n, int i) {
  ...
  result = n.(PostOrderNode).(Expression).getChild(i)
  // and
  // not n instanceof Solidity::AssignmentExpression 
  // and
  // not n instanceof Solidity::CallExpression 
  // and
  ...
  or
  toTreeSitter(n) =
    any(Solidity::AssignmentExpression a |
      i = 0 and toTreeSitter(result) = a.getRight()
      or
      i = 1 and toTreeSitter(result) = a.getLeft()
    )
  or
  n =
    any(Call c |
  ...
  //   )
  // or
  n = any(ExpressionStatement e | i = 0 and result = e.getExpr())  //TODO: implement getStatement()
  or
  // n =
  ...
  // or
  // n =
  //   any(Solidity::VariableDeclarationStatement s |
...
private predicate straightLineSparse(Node scope, int i, Node ni, Spec spec) {
  ...
  // or
  toTreeSitter(scope) =
    any(Solidity::ReturnStatement ret |
      i = -1 and toTreeSitter(ni) = ret and spec.isAt()
      or
      // i = 0 and ni = ret.getExpr() and spec.isAround()
      i = 0 and toTreeSitter(ni) = ret.getChild() and spec.isAround()
      or
      i = 1 and toTreeSitter(ni) = ret and spec.isAroundDestructors()
      // or
  ...
  // or
  toTreeSitter(scope) =
    any(Solidity::ForStatement s |
      // ForStmt [-> init]
      i = -1 and toTreeSitter(ni) = s and spec.isAt()
      or
      i = 0 and toTreeSitter(ni) = s.getInitial() and spec.isAround()
      or
      if exists(s.getCondition())
      then (
        // ... -> before condition
        i = 1 and toTreeSitter(ni) = s.getCondition() and spec.isBefore()
        or
        // body [-> update] -> before condition
        i = 2 and /* BARRIER */ toTreeSitter(ni) = s and spec.isBarrier()
        or
        // i = 3 and ni = s.getStmt() and spec.isAfter()
        // or
        i = 4 and toTreeSitter(ni) = s.getUpdate() and spec.isAround()
        or
        // Can happen when the condition is a `ConditionDeclExpr`
        i = 5 and toTreeSitter(ni) = s.getUpdate() and spec.isAroundDestructors()
        or
        i = 6 and toTreeSitter(ni) = s.getCondition() and spec.isBefore()
        or
        i = 7 and /* BARRIER */ toTreeSitter(ni) = s and spec.isBarrier()
        or
        // i = 8 and ni = s and spec.isAfterDestructors()
        // or
        i = 9 and toTreeSitter(ni) = s and spec.isAfter()
      ) else (
        // ... -> body [-> update] -> before body
        // i = 1 and ni = s.getStmt() and spec.isAround()
        // or
        i = 2 and toTreeSitter(ni) = s.getUpdate() and spec.isAround()
        // or
...
private predicate subEdge(Pos p1, Node n1, Node n2, Pos p2) {
  ...
  // exists(Solidity::WhileStatement s |
  //   p1.nodeAt(n1, s) and
  //   p2.nodeBefore(n2, s.getCondition())
  //   or
 ...
 */
  private predicate conditionJumpsTop(Expression test, boolean truth, Node n2, Pos p2) {
 ...
 */
private predicate conditionJumps(Expression test, boolean truth, Node n2, Pos p2) {
  ...
  // not conditionJumpsTop(test, _, _, _) 
  // and
  // test instanceof ShortCircuitOperator and
  // p2.nodeAfter(n2, test) 
  //   and
  ...
  exists(IfStmt e | test = e.getCondition() |
    truth = true and
...
private predicate conditionalSuccessor(Node n1, boolean truth, Node n2) {
  ...
  exists(Node test, Node targetNode, Pos targetPos |
    precedesCondition(n1, any(Pos at | at.isAt()), test) 
    and
    conditionJumps(test, truth, targetNode, targetPos) 
    and
