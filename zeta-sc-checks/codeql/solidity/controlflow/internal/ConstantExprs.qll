...
private module Cached {
  ...
  module ControlFlowGraphPublic {
    ...
    cached
    predicate conditionAlwaysTrue(Expression condition) { conditionAlways(condition, true) }

    /** Holds if `condition` always evaluates to zero. */
    cached
    predicate conditionAlwaysFalse(Expression condition) {
     ...
     */
    cached
    predicate loopConditionAlwaysTrueUponEntry(ControlFlowNode loop, Expression condition) {
      loopConditionAlwaysUponEntry(loop, condition, true)
    }
  }
}

private predicate conditionAlways(Expression condition, boolean b) {
...
private predicate loopConditionAlwaysUponEntry(ControlFlowNode loop, Expression condition, boolean b) {
 ...
 */
pragma[noinline]
private predicate isFunction(@solidity_ast_node n) {
  n instanceof @solidity_function_definition
  or
  n.(Solidity::Expression).getParent() = n
}
...
private predicate callHasNoTarget(FunctionCall fc) {
  exists(@solidity_function_definition f |
    funbind(fc, f) and
    not isFunction(f)
  )
}

//added by me as its not represented in the db
private predicate funbind(Call fc, Solidity::FunctionDefinition f){
  f.getName().toString() = toTreeSitter(fc).(Solidity::CallExpression).getFunction().getChild().(Solidity::Identifier).getValue().toString()
}

// Pulled out for performance. See
// https://github.com/github/codeql-coreql-team/issues/1044.
private predicate potentiallyReturningFunctionCall_base(Call fc) {
  // fc.isVirtual()
  // or
  callHasNoTarget(fc)
}
...
private predicate potentiallyReturningFunctionCall(Call fc) {
...
//  */
// private predicate impossibleFalseEdge(Solidity::Expression condition, Node succ) {
 ...
 */
private predicate impossibleTrueEdge(Expression condition, Node succ) {
...
private predicate reachableRecursive(ControlFlowNode n) {
  // exists(Solidity::FunctionDefinition f | f.getEntryPoint() = n)
  exists(Function f | f.getBlock() = n)
  or
...
// //EXPERIMENTO: Se puede hacer asi?
// private class TExpression = @solidity_assignment_expression or @solidity_augmented_assignment_expression or @solidity_binary_expression or @solidity_call_expression or @solidity_expression or @solidity_expression_statement or @solidity_inline_array_expression or @solidity_member_expression or @solidity_meta_type_expression or @solidity_new_expression or @solidity_parenthesized_expression or @solidity_payable_conversion_expression or @solidity_struct_expression or @solidity_ternary_expression or @solidity_tuple_expression or @solidity_type_cast_expression or @solidity_unary_expression or @solidity_update_expression;
...
class ExprEvaluator extends int {
  ...
  abstract predicate interesting(Expression e);

  /** Gets the value of (interesting) expression `e`, if any. */
  int getValue(Expression e) { result = this.getValueInternal(toTreeSitter(e), toTreeSitter(e)) }
  ...
  //  */
  // predicate interestingInternal(Solidity::Expression e, Solidity::Expression req, boolean sub) {
  //   this.interesting(e) and req = e and sub = true
  //   or
  //   exists(Solidity::Expression mid | this.interestingInternal(e, mid, sub) |
...
//   }
  ...
  private int getValueInternal(Solidity::Expression e, Solidity::Expression req) {
    //TODO: HACKY! Support other kinds of evaluations
    e.getChild().(Solidity::BooleanLiteral).getChild().(Solidity::Token).getValue().toInt() = 2 and
    result = 0
    or
    e.getChild().(Solidity::BooleanLiteral).getChild().(Solidity::Token).getValue().toInt() = 17 and
    result = 1
...
class ConditionEvaluator extends ExprEvaluator {
  ...
  override predicate interesting(Expression e) {
...
class LoopEntryConditionEvaluator extends ExprEvaluator {
  ...
  abstract override predicate interesting(Expression e);

  /** Holds if `cfn` is the entry point of the loop for which `e` is the condition. */
  abstract predicate isLoopEntry(Expression e, Node cfn);

  /** Holds if `s` is the loop body guarded by the condition `e`. */
  // abstract predicate isLoopBody(Expression e, StmtParent s);
  abstract predicate isLoopBody(Expression e, Stmt s);

  // private predicate isLoopBodyDescendant(Expression e, StmtParent s) {
  private predicate isLoopBodyDescendant(Expression e, Stmt s) {
    this.isLoopBody(e, s)
    or
    // exists(StmtParent mid | this.isLoopBodyDescendant(e, mid) |
    exists(Stmt mid | this.isLoopBodyDescendant(e, mid) |
      s = mid.(Stmt).getAChild() 
      // or
      // s = mid.(Expression).getAChild()
    )
  }

  // Same as `interestingInternal(e, sub, true)` but avoids negative recursion
  private predicate interestingSubExpr(Expression e, Expression sub) {
    this.interesting(e) and e = sub
    or
    exists(Expression mid | this.interestingSubExpr(e, mid) and sub = mid.getAChild())
  }

  private predicate maybeInterestingVariable(Expression e, Variable v) {
   ...
   * definition of `v`.
   */
  private predicate reachesLoopEntryFromLoopBody(Expression e, Variable v, Stmt valueOrDef) {
    this.maybeInterestingVariable(e, v) and
    // (valueOrDef = v.getAnAssignedValue() 
    // or
    // nonAnalyzableVariableDefinition(v, valueOrDef)) 
    // and
  ...
  private predicate loopEntryAt(PrimitiveBasicBlock bb, int pos, Expression e) {
    exists(Node cfn |
   ...
   */
  private predicate bbReachesLoopEntry(PrimitiveBasicBlock bb, Expression e, Variable v) {
  ...
  private predicate bbReachesLoopEntryLocally(PrimitiveBasicBlock bb, Expression e, Variable v) {
    exists(int pos |
