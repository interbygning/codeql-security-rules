...
import codeql.solidity.controlflow.StackVariableReachability
import codeql.solidity.dataflow.EscapesTree
 ...
 */
predicate definitionUsePair(StackVariable var, Expression def, Expression use) {
  exists(Use u |
    u = use and
 ...
 */
predicate definitionReaches(Expression def, Expression node) { def.(Def).reaches(true, _, node) }

// private predicate hasAddressOfAccess(StackVariable var) {
 ...
 * a definition of `var`).
 */
predicate useUsePair(StackVariable var, Expression first, Expression second) {
  // (
  //   /*
...
class DefOrUse extends ControlFlowNodeBase {
   ...
   */
  abstract StackVariable getVariable(boolean isDef);

  pragma[noinline]
  private predicate reaches_helper(boolean isDef, StackVariable v, BasicBlock bb, int i) {
   ...
   */
  cached
  predicate reaches(boolean isDef, StackVariable v, DefOrUse defOrUse) {
    /*
  ...
  private predicate firstBarrierAfterThis(boolean isDef, int j, StackVariable v) {
...
class Def extends DefOrUse {
  ...
  override StackVariable getVariable(boolean isDef) {
...
private predicate bbUseAt(BasicBlock bb, int i, StackVariable v, Use use) {
  bb.getNode(i) = use and
  use.getVariable(false) = v
}

private predicate bbDefAt(BasicBlock bb, int i, StackVariable v, Def def) {
  bb.getNode(i) = def and
  def.getVariable(true) = v
}

private predicate bbBarrierAt(BasicBlock bb, int i, StackVariable v, ControlFlowNode node) {
...
private predicate bbSuccessorEntryReachesDefOrUse(
  BasicBlock bb, StackVariable v, DefOrUse defOrUse,
 ...
 */
predicate definitionBarrier(StackVariable v, ControlFlowNode barrier) {
 ...
 */
predicate definition(StackVariable v, Expression def) {
  def = v.getInitializer().getExpr()
  or
  // variableAccessedAsValue(v.getAnAccess(), def.(AssignExpr).getLValue().getFullyConverted())
  variableAccessedAsValue(v.getAnAccess(), def.(AssignExpr).getLValue())
  // or
 ...
 */
predicate exprDefinition(StackVariable v, ControlFlowNode def, Expression e) {
...
//  */
// predicate definitionByReference(VariableAccess va, Expression def) {
//   exists(Call c, int i |
 ...
 * operations.
 */
predicate useOfVar(StackVariable v, VariableAccess use) {
  use = v.getAnAccess() and
  not exists(AssignExpr e | e.getLValue() = use) 
  // and
 ...
 */
predicate useOfVarActual(StackVariable v, VariableAccess use) {
  useOfVar(v, use) and
  exists(Expression e |
    variableAccessedAsValue(use, e) and
    // not exists(AssignExpr assign | e = assign.getLValue().getFullyConverted())
    not exists(AssignExpr assign | e = assign.getLValue())
  ) and
