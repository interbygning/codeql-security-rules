...
import codeql.solidity.controlflow.DefinitionsAndUses
...
abstract class StackVariableReachability extends string {
  ...
  private predicate bbSuccessorEntryReaches(
    BasicBlock bb, StackVariable v, ControlFlowNode node,
  ...
  private int firstBarrierIndexIn(BasicBlock bb, StackVariable v) {
...
private predicate bbLoopConditionAlwaysTrueUponEntrySuccessor(
  ...
  exists(Expression cond |
    loopConditionAlwaysTrueUponEntry(_, cond) and
    cond.getAChild*() = pred.getEnd() and
    succ = pred.getASuccessor() and
    // not cond.getAChild*() = succ.getStart() and  //VER. Esto queda incompatible
    (
