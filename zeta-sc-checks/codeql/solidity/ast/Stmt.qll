...
/**
 * A Solidity statement.
 */
class Stmt extends AstNode, ControlFlowNode, TStmt {
  /** Holds if `e` is the `n`th child of the underlying statement. */
  predicate hasChild(AstNode e, int n) { this.getChild(n) = e }

  /** Gets the enclosing function of this statement, if any. */
  Function getEnclosingFunction() {
    exists( Function f | f.getBlock().getChild(_) = this and result = f)
  }
  ...
  BlockStmt getEnclosingBlock() {
    if
      this.getParentStmt() instanceof BlockStmt 
      // and
  ...
  Stmt getParentStmt() { result.getAChild() = this}
  ...
  Stmt getFollowingStmt() {
    exists(Solidity::BlockStatement b, int i, int k |
      toTreeSitter(this) = b.getChild(i) and
      k > 0 and
      toTreeSitter(result) = b.getChild(i + k) //TODO: esto no necesariamente me trae el inmediato posterior
    )
  }
...
// private class TStmtChild = @solidity_statement_child_type;
 ...
 */
class ExpressionStatement extends Stmt, TExpressionStatement {
  private Solidity::ExpressionStatement node;

  ExpressionStatement(){toTreeSitter(this) = node}
   ...
   */
  Expression getExpr() { toTreeSitter(result) = node.getChild() }
 ...
 * a loop.
 */
// class ControlStructure extends Stmt, TControlStructure, AstNode {
class ControlStructure extends Solidity::AstNode {

  ControlStructure(){this instanceof Solidity::WhileStatement or this instanceof 
    Solidity::DoWhileStatement or this instanceof Solidity::ForStatement or 
   ...
   */
  Solidity::Expression getControllingExpr() { none() } // overridden by subclasses
...
// class IfStmt extends ControlStructure, Solidity::IfStatement {
class IfStmt extends Stmt, TIfStatement {
  private Solidity::IfStatement node;

  IfStmt(){toTreeSitter(this) = node}
  ...
  Expression getControllingExpr() { result = this.getCondition() }
   ...
   */
  Stmt getThen() { toTreeSitter(result) = node.getBody(0) }
   ...
   */
  Stmt getElse() { 
    this.hasElse() and toTreeSitter(result) = node.getBody(1)
  }
   ...
   * ```
   */
  predicate hasElse() { exists(node.getElse()) }
...
private class TLoop = TWhileStatement or TDoWhileStatement or TForStatement;

/**
 * A Solidity loop, that is, either a 'while' loop, a 'for' loop, or a
 * 'do' loop.
 */
// class Loop extends ControlStructure, TLoop {
  class Loop extends Stmt, TLoop {
  
  // Loop(){this instanceof Solidity::WhileStatement or this instanceof Solidity::ForStatement or this instanceof Solidity::DoWhileStatement}
  Loop(){this instanceof TLoop}
  
  /** Gets the condition expression of this loop. */
  Expression getCondition() { none() } // overridden in subclasses
 ...
 * ```
 */
class WhileStmt extends Loop, TWhileStatement{
  private Solidity::WhileStatement node;

  WhileStmt(){toTreeSitter(this) = node}

  override string getAPrimaryQlClass() { result = "WhileStmt" }

  override Expression getCondition() { toTreeSitter(result) = node.getCondition() }

  // override Expr getControllingExpr() { result = this.getCondition() }
  Expression getControllingExpr() { result = this.getCondition() }

  override Stmt getStmt() { toTreeSitter(result) = node.getBody() }
 ...
 * ```
 */
class ReturnStmt extends Stmt, TReturnStatement {
  private Solidity::ReturnStatement node;

  ReturnStmt(){toTreeSitter(this) = node}
 ...
 * ```
 */
class ForStmt extends Loop, TForStatement {
  private Solidity::ForStatement node;

  ForStmt(){node = toTreeSitter(this)}
   ...
   * ```
   */
  Stmt getInitialization() { toTreeSitter(result) = node.getInitial() }
   ...
   * ```
   */
  Expression getUpdate() { toTreeSitter(result) = node.getUpdate() }
  ...
  Declaration getADeclaration() {
    result = this.getInitialization().(DeclStmt).getAVariable()
  }
 ...
 * ```
 */
class DeclStmt extends Stmt, TVariableDeclarationStatement {
  private Solidity::VariableDeclarationStatement node;

  DeclStmt(){toTreeSitter(this) = node}
  ...
  Variable getAVariable() {
    toTreeSitter(result) = node.getChild().(Solidity::VariableDeclaration) or
    toTreeSitter(result) = node.getChild().(Solidity::VariableDeclarationTuple).getChild(_)
  }
