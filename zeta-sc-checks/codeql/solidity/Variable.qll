...
abstract class Variable extends AstNode, TVariable {
  // private Solidity::VariableDeclaration node;

  // Variable(){toTreeSitter(this) = node}

  predicate isMember(){exists(Solidity::ContractDeclaration c | 
    c.getBody().getChild(_) = toTreeSitter(this))}

  override string getAPrimaryQlClass() { result = "Variable" }

  // DeclStmt getDeclarationStatement(){result.get}

  /** Gets the initializer of this variable, if any. */
  // Initializer getInitializer() { result.getDeclaration() = this }
  Initializer getInitializer() {
    exists(DeclStmt t | t.getValue() = result and t.getAVariable() = this)
    // toTreeSitter(this.getParent()).(Solidity::VariableDeclarationStatement).getValue() = toTreeSitter(result) 
    // or
    // toTreeSitter(this.getParent()).(Solidity::VariableDeclarationTuple).getChild()
  }
  ...
  Specifier getASpecifier() {
    result = toTreeSitter(this.getAChild())
  }
  ...
  override Location getLocation() {
    result = toTreeSitter(this).(Solidity::AstNode).getLocation()
  }
...
//  */
// class LocalScopeVariable extends Variable {
...
class StackVariable extends Variable {
  ...
  StackVariable() {
    toTreeSitter(this) instanceof Solidity::VariableDeclaration or
    toTreeSitter(this) instanceof Solidity::Parameter
  }

    override string toString(){result = "Stackvar"}
}
 ...
 */
class LocalVariable extends StackVariable{
  Solidity::VariableDeclaration node;

  LocalVariable(){toTreeSitter(this) = node}
  ...
  Function getFunction() {
    exists(DeclStmt s | s.getAVariable() = this and s.getEnclosingFunction() = result)
    // exists(Variable s, Function f, int h | s.getName() = this.getName() and toTreeSitter(f.getBlock().getChild(h).getAChild()) = toTreeSitter(s) and result = f)
    // or
...
class MemberVariable extends Variable {
  ...
  //ARGI: lo agrego aca
  //TODO: ver si habria que agregarlo en declaration y hacer toda esa separacion
