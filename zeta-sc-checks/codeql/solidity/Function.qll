 ...
 * 
 *  ARGI NOTA: Por ahora no nos metemos con interfaces, entonces se puede asumir que declaracion
 *  y definicion se solapan siempre.
 * 
 */
// class Function extends Declaration, ControlFlowNode, AccessHolder, @function {
class Function extends TFunctionDefinition, ControlFlowNode {
  private Solidity::FunctionDefinition node;
  Function() {node = toTreeSitter(this) }

  string getName() { result = node.getName().getValue() }
  ...
  // }
    ...
    //NOTA ARGI: tomada de Declaration.qll
  ...
  Specifier getASpecifier() {
    ...
    exists(Specifier s | toTreeSitter(this.getChild(_)) instanceof Specifier and result = s)
  }
   ...
   */
  Parameter getParameter(int n) { toTreeSitter(result) = node.getChild(n) }
   ...
   */
  BlockStmt getBlock() {toTreeSitter(result) = node.getBody()}
   ...
   * `FunctionTryStmt`.
   */
  Stmt getEntryPoint() { result = this.getBlock() }
...
// /**
//  * A Solidity non-contract function (a function that is not a member of any
//  * contract). For example, in the following code, `MyFunction` is a
//  * `FreeFunction` but `MyMemberFunction` is not:
//  * ```
//  * void MyFunction() {
//  *   DoSomething();
//  * }
...
//   // FreeFunction() { not this.isMember() }
